...
lines.each_with_index do |line, line_index|
    line.scan(/\d+/) do |match|
      number = match.to_i
      start_position = $~.begin(0)
      end_position = $~.end(0) - 1
    ...
    line.scan(/([=*+\/&#\-%$@])/) do |match|
      symbol = match
      start_positionC = $~.begin(0) 
      end_positionC = $~.end(0)
  
      special_char << {
        symbol: symbol,
        start_positionC: start_positionC,
        line: line_index
      }
    end
  end

  def intersect(num_start, num_end, num_row, symbol_start, symbol_line)
    num_start <= symbol_start + 1 && symbol_start -1 <= num_end && num_row <= symbol_line + 1 && symbol_line - 1 <= num_row
  end
  ...
  special_char.each{ |y|
    ...
    filteredNum = numbers.filter{
        |n|
        intersect(n[:start_position], n[:end_position], n[:line], y[:start_positionC], y[:line])
    }
    if filteredNum.size == 2 
        resultat += filteredNum[0][:number] * filteredNum[1][:number]
    end
  }
