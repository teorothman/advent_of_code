...
#Rules
#If more than 12 red cubes, 13 green cubes, and 14 blue cubes sum+= gameID
# Go over each line and keep track of highest count per color.
# If highest count is higher than allowed - +=sum gameID
...
games = games.map{
    ...
    id = n.split(":")[0].split(" ").last.to_i
    highestRed = 0
    highestGreen = 0
    highestBlue = 0
    
    n.split(":").last.split(";").map{|draw|
        draw.split(",").map{
            |color|
            value = color.split(" ")[0].to_i
            vColor = color.split(" ")[1]
            if vColor == "red" && value > highestRed
                highestRed = value
            end
            if vColor == "green" && value > highestGreen
                highestGreen = value
            end
            if vColor == "blue" && value > highestBlue
                highestBlue = value
            end
    ...
    if highestRed < 13 && highestGreen < 14 && highestBlue < 15
        sum += id
    end
}
