...
subbedInput = input.gsub("one", "on1e").gsub("two", "tw2o").gsub("three", "thr3ee").gsub("four", "fo4ur").gsub("five", "fi5ve").gsub("six", "si6x").gsub("seven", "se7ven").gsub("eight", "ei8ght").gsub("nine", "ni9ne")

inputNoAlpha = subbedInput.delete('a-zA-Z, ""')

eachLine = inputNoAlpha.split("\n")
...
eachLine.each{
    |n| 
        firstDigit = n[0].to_i
        secondDigit = n[-1].to_i
        bothDigit = firstDigit * 10 + secondDigit
        sum += bothDigit
}
print sum
